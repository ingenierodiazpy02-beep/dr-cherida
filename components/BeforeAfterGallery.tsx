  ...
  return (
    ...
    >
      <div className="absolute inset-0">
        <img src={item.afterImage} alt="Despu\u00e9s" className="w-full h-full object-cover" />
      ...
      <div className="absolute top-0 bottom-0 w-1 bg-gold shadow-lg z-10" style={{ left: `${sliderPosition}%`, transform: 'translateX(-50%)' }}>
  ...
  return (
    <section className="py-24 lg:py-32 bg-background relative overflow-hidden">
      <div className="container mx-auto px-6" ref={ref}>
        ...
        >
          <p className="font-sans text-xs tracking-[0.3em] uppercase text-gold mb-4">Resultados Reales</p>
          <h2 className="font-serif text-5xl lg:text-6xl text-foreground mb-6">{"Antes & Despu\u00e9s"}</h2>
          <p className="font-sans text-muted-foreground max-w-2xl mx-auto">
            Descubre las transformaciones de nuestras pacientes. Desliza para ver los resultados.
          </p>
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            >
              <motion.div key={activeIndex} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <p className="font-sans text-xs tracking-[0.2em] uppercase text-gold mb-2">Tratamiento</p>
                <h3 className="font-serif text-4xl text-foreground mb-4">{galleryItems[activeIndex].treatment}</h3>
                <p className="font-sans text-muted-foreground">{galleryItems[activeIndex].description}</p>
              </motion.div>

              <div className="flex items-center gap-4 pt-6">
                <motion.button onClick={prevSlide} className="w-12 h-12 border border-border rounded-full flex items-center justify-center text-foreground hover:border-gold hover:text-gold transition-all duration-300" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <ChevronLeft className="w-5 h-5" />
                </motion.button>
                <div className="flex gap-2">
                  {galleryItems.map((_, index) => (
                    <button key={index} onClick={() => setActiveIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === activeIndex ? "bg-gold w-8" : "bg-border hover:bg-gold/50"}`} />
                  ))}
                </div>
                <motion.button onClick={nextSlide} className="w-12 h-12 border border-border rounded-full flex items-center justify-center text-foreground hover:border-gold hover:text-gold transition-all duration-300" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              ...
              <div className="flex gap-2 sm:gap-3 pt-4 flex-wrap justify-center lg:justify-start">
                {galleryItems.map((item, index) => (
                  <motion.button key={item.id} onClick={() => setActiveIndex(index)} className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-sm overflow-hidden transition-all duration-300 ${index === activeIndex ? "ring-2 ring-gold" : "opacity-60 hover:opacity-100"}`} whileHover={{ scale: 1.05 }}>
                    <img src={item.afterImage} alt={item.treatment} className="w-full h-full object-cover" />
                  </motion.button>
                ))}
              </div>
            </motion.div>
